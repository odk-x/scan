...
Ref<LuminanceSource> ImageReaderSource::create(cv::Mat const& cvImage) {
  //TODO: Is this allocating 4 color channels?
  //If so try to avoid that without segfaulting.
  zxing::ArrayRef<char> matrix(4 * cvImage.rows * cvImage.cols);

  char* m = &matrix[0];
  for (int y = 0; y < cvImage.rows; y++) {
    for (int x = 0; x < cvImage.cols; x++) {
      *m = cvImage.at<char>(y,x);
      m+=4;
    }
  }

  return Ref<LuminanceSource>(new ImageReaderSource(matrix, cvImage.cols, cvImage.rows, 1));
}
